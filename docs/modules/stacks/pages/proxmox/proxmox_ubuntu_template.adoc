...
ATTENTION: This guide assumes, that you have a working proxmox installation with a storage backend for vm images (like LVM-thin) and a bridged network setup to provide the vms with network access. The guide was tested with Proxmox VE 9.0.
...
apt install -y libguestfs-tools supermin qemu-system-x86 qemu-utils \
               isc-dhcp-client iproute2 curl -y
...
[source, bash]
----
# disable ipv6, generated identifier and install ubuntu-driver during start-up
...
----
NOTE: When running with vlan, make sure the node is not running with the same vlan tag (see `7` above) and the proxmox node has vlan aware enabled under "Datacenter > 'node-name' >> Network > vmbr0 > VLAN aware: Yes"
...
# It was created in advance and serves as a pool for harddrive resources.
...
# increase the ssd size - for llm storage 4GB (default) are not a lot
